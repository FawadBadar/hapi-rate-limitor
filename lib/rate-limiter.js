...
class RateLimiter {
  ...
  getBoomError(remaining) {
    const error = this.boomError && this.boomError(remaining / 1000)
    if (!error || !error.isBoom)
      return Boom.tooManyRequests('You have exceeded the request limit')
    return error
  }
  ...
  async handle(request, h) {
    ...
    throw this.getBoomError(request.rateLimit.remaining)
  }
  ...
  isEnabledFor(request) {
    const { enabled = false } = this.routeConfig(request)
  ...
  resolveRequestIdentifier(request) {
    if (!this.isAuthenticated(request)) {
      if (this.hasUserIdInPayload(request)) {
        return this.userIdFromPayload(request);
      }
   ...
   * @returns {Boolean}
   */
  hasUserIdInPayload(request) {
    return !!this.userIdFromPayload(request)
  }

  /**
   * Returns the user’s unique identifier from payload
   * which is used as the rate limit id.
   *
   * @param {Request} request
   *
   * @returns {String}
   */
  userIdFromPayload(request) {
    return request.payload[this.userAttribute]
  }
